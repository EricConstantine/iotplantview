...
export default {
      ...
      columns: [
        {
          ...
          render: (h, params) => {
            ...
            // 	on:{
            // 		click: () => {
            // 			this.$axios.get('/iotplant/sensor/getsensors?pid='+params.row.id, {}, {
            // 				headers: {
                ...
                on: {
                  click: () => {
                    ...
                    this.$Modal.confirm({
                      // loading:true,
                      title: '重置密码确认',
                      content: '<p>你要重置的用户为' + params.row.username + '，要继续重置操作吗？</p>',
                      onOk: function () {
  ...
  methods: {
    ...
    handleSearch (topage) {
      ...
      this.$axios.get('/iotplant/device/mngpagedata?page=' + current + '&size=' + pagesize + '&sort=' + sort + '&keywords=' + keywords, {}, {
		    	headers: {
		        	'Content-Type': 'application/json;charset=utf-8'
		       	}
		    }).then(function (response) {
    ...
    handleExpexcel () {
        	...
        	let keywords = this.table.keywords
        	window.location.href = '/netgate-server/user/expexcel?sort=' + sort + '&size=1000&keywords=' + keywords
    },
        	...
        	if (selection.length > 0) {
        		this.$Modal.confirm({
        			...
        			onOk: function () {
			        	...
			        	this.$axios.delete('/netgate-server/user/delete?usersids=' + usersids, {}, {
					    	headers: {
					        	'Content-Type': 'application/json;charset=utf-8'
					       	}
					    }).then(function (response) {
